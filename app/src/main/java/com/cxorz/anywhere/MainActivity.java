...
public class MainActivity extends BaseActivity implements SensorEventListener {
    ...
    private final float[] mR = new float[9];// Rotation matrix to store magnetic field and acceleration data
    ...
    private void resetMap() {
        if (mLocationOverlay.getMyLocation() != null) {
            ...
            if (!GoUtils.isGpsOpened(this)) {
                ...
                GoUtils.DisplayToast(this, "Location failed: Please ensure you are in an open outdoor area and try again later");
            }
        }
    }
    ...
    private void doGoLocation(View v) {
        ...
        if (isMockServStart) {
            if (mMarkLatLngMap == null) {
                stopGoLocation();
                Snackbar.make(v, "Mock location stopped", Snackbar.LENGTH_LONG)
            ...
            if (!GoUtils.isAllowMockLocation(this)) {
                ...
                if (mMarkLatLngMap == null) {
                    Snackbar.make(v, "Please click a location on the map or search for a location first", Snackbar.LENGTH_LONG)
                            .setAction("Action", null).show();
                } else {
                    startGoLocation();
                    mButtonStart.setImageResource(R.drawable.ic_fly);
                    Snackbar.make(v, "Mock location started", Snackbar.LENGTH_LONG)
                            .setAction("Action", null).show();
